...
import { createVirtualHeightIndexStore } from "@better-scrollbar/core"
import raf from "../raf"
...
export function getResizeObserverEntryHeight(entry: ResizeObserverEntry, element: HTMLElement) {
	...
	if (
		typeof contentRectHeight === "number" &&
		Number.isFinite(contentRectHeight) &&
		contentRectHeight > 0
	) {
...
export default ({
	...
	if (
		...
		heightIndexOptionsRef.current.preserveMeasuredHeightsOnItemCountChange !==
			preserveMeasuredHeightsOnItemCountChange
	) {
		const itemCountChanged = heightIndexOptionsRef.current.itemCount !== safeItemCount
		const shouldPreserveMeasuredHeights =
			!itemCountChanged || preserveMeasuredHeightsOnItemCountChange
	...
	const unobserveKey = useCallback(
		(key: React.Key) => {
	...
	const deleteCachedHeight = useCallback(
		(key: React.Key) => {
	...
	const evictHeightCacheOverflow = useCallback(
		(protectedKey?: React.Key) => {
	...
	const rememberMeasuredHeight = useCallback(
		(key: React.Key, index: number, height: number) => {
			heightsRef.current.delete(key)
	...
	const setInstanceRef = useCallback(
		(key: React.Key, index: number, instance: HTMLElement | null) => {
			...
			if (
				...
				previousIndex !== index &&
				indexKeyRef.current.get(previousIndex) === key
			) {
			...
			const shouldNotifyCachedHeightMove =
	...
	const pruneHeights = useCallback(
		(keys?: React.Key[], itemCount?: number) => {
			...
			heightsRef.current.forEach((_, key) => {
				...
				const removedByIndex =
					itemCount !== undefined && index !== undefined && index >= itemCount
	...
	useEffect(() => {
		const keyElements = keyElementRef.current
		const elementKeys = elementKeyRef.current
		return () => {
			cancelRaf()
