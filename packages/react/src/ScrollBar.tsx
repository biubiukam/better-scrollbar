...
import {
	...
	getAdaptiveOverscanOptions,
	getDeviceScale,
	getEffectiveOverscan,
	getLogicalScrollWindowStart,
	getNextStickyIndex,
	getSafeBrowserScrollHeight,
	getSafeMaxRenderedItems,
	getStickyOverlayOffset,
	isSameItemsRenderedInfo,
	isSameScrollState,
	normalizeStickyIndices,
	resolveFollowOutputThreshold,
...
const ScrollBar = forwardRef<VirtualScrollBarRef, PropsWithChildren<VirtualScrollBarProps>>(
	(props, ref) => {
		const {
			onScrollStart,
			onScrollEnd,
		...
		const resolvedAdaptiveOverscan =
			adaptiveOverscanProp !== undefined ? adaptiveOverscanProp : overscanConfig.adaptive
		const followOutputThreshold = resolveFollowOutputThreshold(followOutput)

		const useIndexedRendering =
			typeof itemCount === "number" && typeof renderItem === "function"
		...
		const itemRefCallbackCacheRef = useRef(
			new Map<
				React.Key,
				{
					index: number
		...
		const updateScrollState = useCallback(
			(updater: (draft: ScrollState) => void, sync = false) => {
		...
		const safeMaxRenderedItems = getSafeMaxRenderedItems(
			maxRenderedItemsProp ?? DEFAULT_MAX_RENDERED_ITEMS
		)
		...
		const [stickyTransitionDistance, setStickyTransitionDistance] =
			useState<StickyTransitionDistance | null>(null)
		...
		const effectiveOverscan = useMemo(() => {
			...
			// eslint-disable-next-line react-hooks/exhaustive-deps -- scrollState fields trigger recalculation intentionally
		...
		const {
			...
			// eslint-disable-next-line react-hooks/exhaustive-deps -- updatedMark signals height changes
		}, [
			clientHeight,
			effectiveOverscan,
			heightIndex,
		...
		const activeStickyIndex = useMemo(() => {
			return getActiveStickyIndex(normalizedStickyIndices, scrollState.y, (index) =>
				heightIndex.getOffset(index)
			)
			// eslint-disable-next-line react-hooks/exhaustive-deps -- updatedMark signals height changes
		...
		const activeStickyOverlayHeight =
			stickyOverlaySize !== null && stickyOverlaySize.index === activeStickyIndex
		...
		const browserScrollHeightLimit = getSafeBrowserScrollHeight(
			maxBrowserScrollHeightProp,
			clientHeight
		)
		const physicalScrollHeight =
			scrollHeight > browserScrollHeightLimit
				? Math.max(clientHeight, browserScrollHeightLimit)
				: scrollHeight
		const effectiveScrollMode =
			scrollModeProp === "native" && scrollHeight <= browserScrollHeightLimit
				? "native"
				: "controlled"
		...
		const physicalToLogicalY = useCallback(
			(scrollTop: number) => {
				const logicalRange = maxScrollHeightRef.current
				const physicalRange = maxPhysicalScrollHeightRef.current
				if (logicalRange <= 0 || physicalRange <= 0) {
		...
		const syncNativeScrollOffset = useCallback(
			...
			(
				...
				if (scrollSeekOptions) {
					const normalizedDelta =
						nextActivity.delta / Math.max(nextActivity.deviceScale, 1)
		...
		const stickyOverlayOffset = useMemo(() => {
			...
			const actualPhysicalScrollTop =
				viewContainerRef.current?.scrollTop ?? expectedPhysicalScrollTop
		...
		const resolveAnchorIndex = useCallback(
			(anchor: ScrollAnchorSnapshot) => {
				...
				const directIndex =
					anchor.index >= 0 && anchor.index < totalItemCount ? anchor.index : -1
				if (directIndex < 0) {
					return -1
				}
				...
				const endIndex = Math.min(
					directIndex + INDEXED_ANCHOR_SEARCH_RADIUS,
					totalItemCount - 1
				)
		...
		useLayoutEffect(() => {
			...
			if (
				...
				previousAnchor &&
				previousAnchor.itemCount > 0 &&
				totalItemCount > 0
			) {
				const scrollOffsetStayedOnAnchor =
					Math.abs(scrollState.y - previousAnchor.scrollY) <= 0.5
		...
		const onUpdateScrollState = useCallback(
			(scrollTop: ScrollOffsetUpdater) => {
				onUpdateScrollOffset({ y: scrollTop }, { sync: true, deferNativeSync: true })
			},
			[onUpdateScrollOffset]
		)

		const onUpdateHorizontalScrollState = useCallback(
			(scrollLeft: ScrollOffsetUpdater) => {
		...
		useEffect(() => {
			...
			const onScroll = function (event: WheelEvent): void {
				...
				const rawScrollOffset = event.shiftKey
					? deltaX || deltaY
				...
				const elapsedMs =
					lastWheelTimeRef.current === null
						? 16
						: Math.max(now - lastWheelTimeRef.current, 1)
					...
					onUpdateScrollOffset(
						{
							...(shouldScrollHorizontal
								? {
										x: (preScrollStateX: number) =>
											keepInHorizontalRange(preScrollStateX + scrollOffset)
									}
								: {
										y: (preScrollStateY: number) =>
											keepInVerticalRange(preScrollStateY + scrollOffset)
									})
						},
						{ elapsedMs }
					)
		...
		const shouldPreserveItemState =
			preserveItemStateProp && totalItemCount <= safeMaxRenderedItems
		...
		const listChildren = useMemo(() => {
			...
			const shouldRenderPlaceholder = Boolean(
				scrollSeekActive && scrollSeekOptions && shouldUseVirtualRendering
			)
			...
			for (let index = renderStart; index <= renderEnd; index++) {
				...
				const setRef = shouldRenderPlaceholder
					? ignoreItemMeasurement
					: getItemRefCallback(key, index)
				renderedNodes.push(
					<Item
						key={key}
						hidden={hidden}
		...
		const stickyOverlay = useMemo(() => {
			...
			const node = useIndexedRendering
				? renderItem?.(activeStickyIndex)
				: childNodes[activeStickyIndex]
			if (!node) {
				return null
			}
			...
			return (
				<div
					...
					style={{
						...
						zIndex: 1,
						height: 0,
						...(stickyOverlayOffset < 0
							? { transform: `translateY(${stickyOverlayOffset}px)` }
							: {})
					}}
				>
					<Item
		...
		const onFallbackScroll = useCallback(
			(event: React.UIEvent<HTMLDivElement>) => {
				const { scrollLeft: newScrollLeft, scrollTop: newPhysicalScrollTop } =
					event.currentTarget
				...
				if (
					Math.abs(newScrollLeft - currentScrollState.x) > 0.5 ||
					Math.abs(newScrollTop - currentScrollState.y) > 0.5
				) {
					const now = Date.now()
					const elapsedMs =
						lastNativeScrollTimeRef.current === null
							? 16
		...
		const physicalFillerOffset =
			logicalToPhysicalY(scrollState.y) + (fillerOffset - scrollState.y)
