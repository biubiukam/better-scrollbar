...
export function StreamCursor() {
	return <span aria-hidden="true" className="ml-1 inline-block h-3 w-1 animate-pulse rounded-sm bg-current align-[-1px]" />
}

function PhaseLabel({ labels, phase }: { labels: AgentComponentLabels, phase: ToolPhase }) {
	const label = phase === "chunk" ? labels.toolChunkPhase : phase === "calling" ? labels.toolCallingPhase : labels.toolResponsePhase
	return (
		<span className="inline-flex items-center gap-1 rounded-md border border-border bg-muted/60 px-1.5 py-0.5 text-[11px] leading-4 text-muted-foreground">
			{ phase === "calling" ? <Loader2 className="h-3 w-3 animate-spin" /> : <CircleDot className="h-3 w-3" /> }
			{ label }
		</span>
	)
}

export function ToolCallCard({ labels, tool, streaming }: { labels: AgentComponentLabels, tool: AgentToolState, streaming?: boolean }) {
	return (
		<div
...
export function ThinkingCard({
	...
	return (
		<div className="agent-thinking-card mt-2 grid gap-1.5" aria-expanded={ expanded ? "true" : "false" }>
			<button
				aria-label={ label }
...
export const AgentMessageRow = React.forwardRef<HTMLDivElement, AgentMessageRowProps>(function AgentMessageRow({
	labels,
	message,
	onToggleThinking,
}, ref) {
	...
	return (
		...
		>
			<div className={ cn("min-w-0 max-w-[86%] rounded-lg border px-3 py-2 shadow-sm", bubbleClass) }>
				<div className={ cn("flex min-w-0 items-center gap-2", isUser && "justify-end") }>
					<span className={ cn("shrink-0 font-semibold", isUser ? "text-primary-foreground" : "text-card-foreground") }>
						{ message.title }
					</span>
					<span className={ cn(
						"rounded-md border px-1.5 py-0.5 text-[11px] leading-4",
						isUser ? "border-primary-foreground/30 bg-primary-foreground/10 text-primary-foreground" : "border-border bg-muted/40 text-muted-foreground"
					) }>
						{ message.meta } · { message.timestamp }
					</span>
				</div>
				{ message.kind === "thinking" ? (
					<ThinkingCard labels={ labels } message={ message } onToggle={ () => onToggleThinking(message.id) } />
				) : null }
				{ message.kind === "tool" && message.tool ? (
					<ToolCallCard labels={ labels } tool={ message.tool } streaming={ message.streaming } />
				) : null }
				{ message.kind === "assistant" || message.kind === "user" ? (
					<p className={ cn("agent-message-body mt-1 whitespace-pre-line text-xs leading-5", isUser ? "text-primary-foreground/90" : "text-muted-foreground") }>
						{ message.content }
						{ message.streaming ? <StreamCursor /> : null }
					</p>
				) : null }
