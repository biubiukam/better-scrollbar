...
function AuditRow({ copy, index }: { copy: ExampleCopy["scenarioDemos"]["audit"], index: number }) {
	...
	const status = tone === "alert" ? copy.rows.risk : index % 9 === 0 ? copy.rows.settlement : copy.rows.cleared
	const channel = index % 3 === 0 ? copy.rows.ledger : index % 3 === 1 ? copy.rows.transfer : copy.rows.settlement
	...
	return (
		...
		>
			...
			<span className={demoTw.rowTitle}>{channel} · {status}</span>
			<span className={demoTw.rowMeta}>trace_{(index % 10_000).toString().padStart(4, "0")}</span>
		</div>
	)
}
...
export function AuditLogDemo({ copy = DEFAULT_EXAMPLE_COPY }: AuditLogDemoProps) {
	...
	const jumpTo = useCallback((ratio: number) => {
		...
		scrollerRef.current?.scrollTo({ x: 0, y: getJumpOffset(state.scrollHeight, state.clientHeight, ratio) })
	}, [scrollState])
	...
	return (
		<div className={cn(demoTw.shell, "audit-demo-wrapper")}>
			...
			<footer className={cn(demoTw.toolbar, "audit-demo-toolbar")}>
				...
				<ActionButton icon={<ArrowDown className="h-3.5 w-3.5" />} onClick={() => jumpTo(1)}>
