...
function AnchorMutation({ copy = DEFAULT_EXAMPLE_COPY }: { copy?: ExampleCopy }) {
	...
	const renderItem = useCallback((index: number) => {
		if (index < prependedCount) {
			return (
				<div className={ cn("optimization-row optimization-row--focus anchor-mutation-row", caseTw.row, toneRowTw.focus) } style={ {height: ESTIMATED_MILLION_ROW_HEIGHT} }>
					<div className={ cn("optimization-row-main", caseTw.rowMain) }>
						<span className={ caseTw.rowIndex }>{ caseCopy.history }</span>
						<strong className={ caseTw.rowTitle }>{ caseCopy.batch(prependedCount - index) }</strong>
					</div>
					<div className={ cn("optimization-row-meta", caseTw.rowMeta) }>{ caseCopy.anchor }</div>
				</div>
			)
		}
		...
		return (
			<div className={ cn("optimization-row anchor-mutation-row", `optimization-row--${ tone }`, caseTw.row, toneRowTw[tone]) } style={ {height} }>
				<div className={ cn("optimization-row-main", caseTw.rowMain) }>
	...
	return (
		<article
			className={ cn("optimization-case-card anchor-mutation-card", caseTw.card, caseTw.accentWarning) }
		...
		>
			<header className={ cn("optimization-case-head", caseTw.head) }>
				...
				<p className={ caseTw.desc } data-testid="case-highlight">{ caseCopy.desc }</p>
			...
			<div className={ cn("optimization-case-body", caseTw.body) }>
				<div className={ cn("optimization-case-toolbar", caseTw.toolbar) }>
					<button className={ caseTw.button } type="button" onClick={ jumpToMiddle }>{ caseCopy.jumpMiddle }</button>
					<button className={ caseTw.button } type="button" onClick={ prependRows }>{ caseCopy.prepend }</button>
				</div>
				<div className={ cn("optimization-case-list", caseTw.list) }>
					<VirtualScrollBar
						...
						itemKey={ (index) => index < prependedCount ? `history-${ index }` : `anchor-mutation-${ index - prependedCount }` }
				...
				<div className={ cn("optimization-case-stats", caseTw.stats) }>
					<span>{ copy.shared.visible } { formatVirtualRange({
						startIndex: itemsRendered.visibleStartIndex,
						endIndex: itemsRendered.visibleEndIndex
					}) }</span>
					<span>{ copy.shared.rendered } { formatVirtualRange(itemsRendered) }</span>
					<span>{ copy.shared.total } { totalCount.toLocaleString() }</span>
				</div>
			</div>
		</article>
	)
