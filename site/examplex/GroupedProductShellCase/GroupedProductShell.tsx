			...
			return (
				<div className={ cn("optimization-row optimization-group-row grouped-product-shell-row", caseTw.row, caseTw.groupRow) } style={ {height: 44} }>
					<div className={ cn("optimization-row-main", caseTw.rowMain) }>
						<span className={ caseTw.rowIndex }>{ caseCopy.group(groupInfo.groupIndex + 1) }</span>
						<strong className={ caseTw.rowTitle }>{ caseCopy.items(groupInfo.groupItemCount) }</strong>
					</div>
					<div className={ cn("optimization-row-meta", caseTw.rowMeta) }>{ caseCopy.sticky }</div>
				</div>
			)
		}
		...
		>
			...
			<div className={ cn("optimization-case-body", caseTw.body) }>
				<div className={ cn("optimization-case-toolbar", caseTw.toolbar) }>
					<button className={ caseTw.button } type="button" onClick={ jumpGroup }>{ caseCopy.jumpGroup }</button>
				</div>
				...
				<div className={ cn("optimization-case-stats", caseTw.stats) }>
					...
					<span>{ copy.shared.y } { Math.round(scrollState.y).toLocaleString() }</span>
				</div>
			</div>
		</article>
	)
