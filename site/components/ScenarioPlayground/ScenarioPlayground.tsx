...
import VirtualScrollBar, { getStickyIndicesFromGroups } from "../../../src"
import type { ItemsRenderedInfo, OverscanConfig, ScrollState, VirtualScrollBarRef } from "../../../src"
...
function ScenarioPlayground({ copy = DEFAULT_EXAMPLE_COPY }: { copy?: ExampleCopy }) {
	...
	const scenarioStickyIndices = useMemo(() => getStickyIndicesFromGroups(scenarioGroupCounts), [scenarioGroupCounts])
	const toneChannel = getToneChannel(scrollState)

	useEffect(() => {
		setLogs([copy.scenario.logs.initial])
	...
	const deleteVisibleRow = useCallback(() => {
		...
		recordLog(copy.scenario.logs.deleteVisibleRow((itemsRenderedRef.current.visibleStartIndex + 1).toLocaleString()))
	...
	const renderPlaceholder = useCallback((index: number) => (
		<div className="scenario-playground-placeholder grid w-full grid-cols-[132px_minmax(0,1fr)_94px_54px] items-center gap-2.5 border-t border-border bg-card px-3.5 text-[13px] text-muted-foreground opacity-70 tabular-nums" style={ {height: getScenarioEstimatedHeight(config)} }>
			...
			<strong className="min-w-0 truncate font-medium">{ copy.scenario.rows.scrolling }</strong>
		</div>
	...
	const renderItem = useCallback((index: number) => {
		...
		if (isGroupHeader) {
			...
			return (
				...
				>
					<span className="font-semibold text-card-foreground">{ copy.scenario.rows.group(groupIndex + 1) }</span>
					<strong className="min-w-0 truncate font-medium text-card-foreground">{ (SCENARIO_GROUP_BLOCK_SIZE - 1).toLocaleString() } { copy.scenario.rows.rows }</strong>
					<em className="not-italic text-xs text-muted-foreground">{ copy.scenario.rows.sticky }</em>
				</div>
			)
		}
		...
		return (
			<div
				...
				onDragEnd={ onNativeDragEnd }
			>
				<span className="font-semibold text-card-foreground">{ isPrepended ? copy.scenario.rows.history : `#${ (businessIndex + 1).toLocaleString() }` }</span>
				<strong className="min-w-0 truncate font-medium">{ isPrepended ? copy.scenario.rows.prependedBatch(prependedCount - orderedIndex) : copy.scenario.rows.order((businessIndex % 997) + 1) }</strong>
				<em className="not-italic text-xs text-muted-foreground">{ isPrepended ? copy.scenario.rows.anchor : getMillionRowStatus(businessIndex) }</em>
	...
	return (
		<div className="scenario-playground flex w-full flex-col bg-card text-card-foreground" data-row-count={ MILLION_ROW_COUNT }>
			<div className="scenario-playground-head grid flex-none gap-3 border-b border-border px-5 py-4 lg:flex lg:items-start lg:justify-between lg:gap-[18px]">
				<div>
					<div className="scenario-playground-kicker text-xs font-semibold leading-[18px] text-accent">{ copy.scenario.kicker }</div>
					<h3 className="mt-1 text-lg font-semibold leading-[26px] text-card-foreground">{ copy.scenario.title }</h3>
					<p className="mt-1.5 max-w-[760px] text-[13px] leading-5 text-muted-foreground">{ copy.scenario.desc }</p>
				</div>
				<div className="scenario-playground-total grid min-w-[140px] gap-0.5 rounded-md border border-border bg-muted/30 px-2.5 py-2 text-left tabular-nums lg:text-right">
					<span className="text-xs leading-4 text-muted-foreground">{ copy.scenario.baseRows }</span>
					<strong className="text-[15px] font-semibold leading-5 text-card-foreground">{ MILLION_ROW_COUNT.toLocaleString() } { copy.shared.rows }</strong>
				</div>
			</div>
			<div className="scenario-playground-grid grid grid-cols-1 lg:grid-cols-[minmax(280px,340px)_minmax(0,1fr)]">
				<aside className="scenario-playground-controls min-w-0 border-b border-border bg-muted/20 p-3.5 lg:border-b-0 lg:border-r" aria-label={ copy.scenario.controlsLabel }>
					...
					<p className="scenario-playground-preset-desc mt-2.5 min-h-14 text-xs leading-[18px] text-muted-foreground">{ activePresetCopy.description }</p>
					<div className="scenario-playground-form grid gap-2.5">
						<Field label={ copy.scenario.fields.heightMode }>
							...
							>
								...
								<option value="dynamic">{ copy.scenario.heightOptions.dynamic }</option>
							</select>
						</Field>
						...
						<Field label={ copy.scenario.fields.overscanPixels }>
							...
							>
								<option value="standard">{ copy.scenario.styleOptions.standard }</option>
						...
						<div className="scenario-playground-toggle-grid grid grid-cols-2 gap-2">
							<Toggle label={ copy.scenario.toggles.adaptiveOverscan } checked={ config.adaptiveOverscan } onChange={ (checked) => updateConfig({adaptiveOverscan: checked}) }/>
							<Toggle label={ copy.scenario.toggles.scrollSeek } checked={ config.scrollSeek } onChange={ (checked) => updateConfig({scrollSeek: checked}) }/>
							<Toggle label={ copy.scenario.toggles.maintainVisibleContentPosition } checked={ config.maintainVisibleContentPosition } onChange={ (checked) => updateConfig({maintainVisibleContentPosition: checked}) }/>
							<Toggle label={ copy.scenario.toggles.followOutput } checked={ config.followOutput } onChange={ (checked) => updateConfig({followOutput: checked}) }/>
							<Toggle label={ copy.scenario.toggles.grouped } checked={ config.grouped } onChange={ (checked) => updateConfig({grouped: checked}) }/>
							<Toggle label={ copy.scenario.toggles.dragEnabled } checked={ config.dragEnabled } onChange={ (checked) => updateConfig({dragEnabled: checked}) }/>
						</div>
					</div>
				</aside>
				<section className="scenario-playground-stage flex min-w-0 flex-col bg-card">
					<div className="scenario-playground-actions flex min-h-12 shrink-0 flex-wrap items-center gap-2 border-b border-border bg-card px-3 py-2">
						...
						<button className={ SCENARIO_BUTTON_CLASS } type="button" onClick={ fastScroll }>{ copy.scenario.actions.fastScroll }</button>
						<button className={ SCENARIO_BUTTON_CLASS } type="button" aria-label={ copy.scenario.actions.prependRowsAria } onClick={ prependRows }>{ copy.scenario.actions.prependRowsLabel }</button>
						<button className={ SCENARIO_BUTTON_CLASS } type="button" aria-label={ copy.scenario.actions.appendRowsAria } onClick={ appendRows }>{ copy.scenario.actions.appendRowsLabel }</button>
						<button className={ SCENARIO_BUTTON_CLASS } type="button" onClick={ deleteVisibleRow }>{ copy.scenario.actions.deleteVisibleRow }</button>
						<button className={ SCENARIO_BUTTON_CLASS } type="button" onClick={ resetMutations }>{ copy.scenario.actions.resetMutations }</button>
					</div>
					...
					<div className="scenario-playground-metrics grid shrink-0 grid-cols-2 gap-px border-b border-border bg-border lg:grid-cols-5">
						<Metric label={ copy.scenario.metrics.total } value={ itemCount.toLocaleString() }/>
						<Metric label={ copy.scenario.metrics.visible } value={ formatVirtualRange({
							startIndex: itemsRendered.visibleStartIndex,
							endIndex: itemsRendered.visibleEndIndex
						}) }/>
						<Metric label={ copy.scenario.metrics.rendered } value={ formatVirtualRange(itemsRendered) }/>
						<Metric label={ copy.scenario.metrics.y } value={ Math.round(scrollState.y).toLocaleString() }/>
					...
					<div className="scenario-playground-bottom grid min-h-0 flex-1 grid-cols-1 border-b border-border lg:grid-cols-[minmax(0,1.3fr)_minmax(220px,0.7fr)]">
						<div className="scenario-playground-snapshot min-w-0 border-b border-border px-3.5 py-3 lg:border-b-0 lg:border-r">
							<h4 className={ SCENARIO_PANEL_TITLE_CLASS }>{ copy.scenario.panels.propsSnapshot }</h4>
						...
						<div className="scenario-playground-log min-w-0 px-3.5 py-3">
							<h4 className={ SCENARIO_PANEL_TITLE_CLASS }>{ copy.scenario.panels.eventLog }</h4>
