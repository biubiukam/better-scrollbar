	...
	en: {
		...
		scenarioDemos: {
			agent: {
				...
				subtitle:
					"A six-message incident conversation with one-click simulation, collapsible thinking, structured tools and forced bottom following.",
				actions: {
				...
				rows: {
					...
					thinking: "Thinking",
					thinkingDetails:
						"Reading trace samples, deployment diffs, retry-policy changes, payment authorization spans, user impact notes and rollback thresholds before writing a safe recommendation.",
			...
			media: {
				...
				subtitle:
					"scrollSeek, adaptive overscan and expensive preview rows keep heavy result browsing responsive.",
				actions: {
			...
			rules: {
				...
				subtitle:
					"Virtual rows keep drag sorting, row status and queue operations interactive inside the visible range.",
				actions: {
					enqueue: "Enqueue",
		...
		scenario: {
			...
			presets: {
				baseline: {
					label: "Base list",
					description:
						"Fixed heights and basic overscan show the default cost of indexed rendering at 100M rows."
				},
				dynamic: {
					label: "Dynamic height",
					description:
						"Measure visible row heights and validate how estimates plus cache backfill affect scroll stability."
				},
				fast: {
					label: "Fast scroll",
					description:
						"Adaptive overscan and scroll placeholders expose range stability and blank-space risk under high velocity."
				},
				chat: {
					label: "Chat append",
					description:
						"Bottom following and anchored positioning simulate history inserts and new message appends."
				},
				grouped: {
					label: "Grouped grid",
					description:
						"groupCounts, grid ARIA and a safe physical scroll height validate massive table semantics."
				},
				drag: {
					label: "Drag sorting",
					description:
						"Drag only the current visible window to verify virtual rows and row interactions can coexist."
				},
				styled: {
					label: "Custom scrollbar",
					description:
						"Custom view and scrollbar styling show lightweight progress-driven visual updates."
				}
			},
			fields: {
	...
	zh: {
		...
		scenarioDemos: {
			agent: {
				...
				subtitle:
					"6 条真实 Agent 事故会话，支持一键模拟、可折叠思考、结构化工具调用和强制底部跟随。",
				actions: {
					scrollTop: "滚动到顶部",
					scrollBottom: "滚动到底部",
				...
				rows: {
					...
					thinking: "思考中",
					thinkingDetails:
						"正在读取追踪样本、部署差异、重试策略变更、支付授权热点、用户影响面和回滚阈值，再把可验证证据整理成安全建议。",
					thinkingCollapse: "收起",
					thinkingExpand: "展开",
					toolInput: "工具输入",
					toolOutput: "工具输出",
			...
			media: {
				title: "富媒体素材库 / 搜索结果流",
				subtitle:
					"用 scrollSeek、自适应 overscan 和重行渲染优化，支撑图片预览密集的搜索结果流。",
				actions: {
					switchQuery: "切换搜索词",
					fastScroll: "快速滚动",
					toggleDensity: "切换密度",
		...
		scenario: {
			...
			presets: {
				baseline: {
					label: "基础大列表",
					description:
						"固定高度、基础 overscan，观察 1 亿行 indexed rendering 的默认成本。"
				},
				dynamic: {
					label: "动态高度",
				...
				fast: {
					label: "快速滚动",
					description:
						"启用自适应 overscan 与滚动占位，观察高速滚动下区间稳定性和空白风险。"
				},
				chat: {
					label: "聊天追加",
				...
				grouped: {
					label: "分组表格",
					description:
						"启用 groupCounts、grid ARIA 和安全物理滚动高度，验证超大表格语义。"
				},
				drag: {
					label: "拖拽排序",
