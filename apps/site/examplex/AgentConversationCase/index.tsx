...
import type { ItemsRenderedInfo, ScrollState, VirtualScrollBarRef } from "@better-scrollbar/react"
...
import { AGENT_STREAM_CHUNK_MS, MOCK_AGENT_CONVERSATION, createMockAgentStream } from "./mockData"
...
function getLocaleFromCopy(
	copy: ExampleCopy | undefined,
	explicitLocale: LocaleMode | undefined
): LocaleMode {
	if (explicitLocale) {
...
function upsertMessage(
	...
	return messages.map((message, index) =>
		index === existingIndex ? updateMessage(message) : message
	)
}
...
export function AgentConversationDemo({
	...
	const [messages, setMessages] = useState<AgentMessage[]>(() =>
		createCompletedMessages(conversation)
	)
	...
	const keepLatestOutputVisible = useCallback(
		(frameCount = BOTTOM_FOLLOW_FRAMES) => {
			if (bottomFollowFrameRef.current !== undefined) {
				window.cancelAnimationFrame(bottomFollowFrameRef.current)
	...
	const applyStreamEvent = useCallback(
						...
						(message) => {
							...
							return {
								...
								tool: {
									...
									request:
										event.phase === "chunk"
											? `${tool.request}${event.delta}`
											: tool.request,
									response:
										event.phase === "response"
											? `${tool.response}${event.delta}`
											: tool.response
								}
							}
						}
		...
		[stopTimer, ui.labels.thinkingTitle, ui.logComplete]
	)
