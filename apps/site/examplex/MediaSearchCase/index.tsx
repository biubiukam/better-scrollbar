	...
	return (
		...
		>
			...
			<span className={demoTw.rowMeta}>
				{index % 6 === 0 ? copy.rows.heavy : copy.rows.preview}
			</span>
		</div>
	)
}
...
export function MediaSearchDemo({ copy = DEFAULT_EXAMPLE_COPY }: MediaSearchDemoProps) {
	...
	const fastScroll = useCallback(() => {
		...
		scrollerRef.current?.scrollTo({
			x: 0,
			y: getJumpOffset(state.scrollHeight, state.clientHeight, 0.68)
		})
	}, [scrollState])
		...
		<div className={cn(demoTw.shell, "media-demo-wrapper")}>
			...
			<div className={cn(demoTw.listTall, "media-demo-list")}>
				<VirtualScrollBar
					...
					overscan={{
						items: 4,
						pixels: { before: 240, after: 520 },
						adaptive: { min: 4, max: 18, velocityFactor: 0.04, timeFactor: 0.2 }
					}}
					scrollSeek={{
