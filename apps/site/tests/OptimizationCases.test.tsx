...
describe("OptimizationCases", () => {
	...
	it("shows the advanced case with sticky grouping and default list metadata", () => {
		...
		const firstGroupCount = Number(
			advancedCase
				?.querySelector(".optimization-group-row strong")
		...
		expect(advancedCase?.querySelector("[role='listitem']")?.getAttribute("aria-setsize")).toBe(
			String(MILLION_ROW_COUNT)
		)
	...
	it("keeps the grouped product shell scrollbar above the sticky row layer", () => {
		const { container } = render(<OptimizationCases />)
		const groupedCase = container.querySelector(
		...
		const trackZIndex = Number.parseInt(
			window.getComputedStyle(verticalTrack).zIndex || "0",
			10
		)
	...
	it("keeps the same business row visible after prepending rows in the anchor case", () => {
		const { container } = render(<OptimizationCases />)
		const anchorCase = container.querySelector(
	...
	it("keeps the top business row anchored when prepending from the start", () => {
		...
		expect(anchorCase.querySelector(".optimization-case-stats")?.textContent).toContain(
			"Visible 20 -"
		)
	})
