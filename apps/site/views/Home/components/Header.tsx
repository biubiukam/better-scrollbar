...
const headerGlassClass =
	"bg-background/60 backdrop-blur-2xl backdrop-saturate-150 supports-[backdrop-filter]:bg-background/50"
...
export function Header({ copy, theme, locale, onThemeChange, onLocaleChange }: HeaderProps) {
	...
	useEffect(() => {
		...
		const updateScrolledState = () => {
			...
			setHasScrolled((currentHasScrolled) =>
				currentHasScrolled === nextHasScrolled ? currentHasScrolled : nextHasScrolled
			)
		}
	...
	return (
		<header
			className={`${headerBaseClass} ${hasScrolled ? headerGlassClass : headerTransparentClass}`}
		>
							...
							<TooltipTrigger asChild>
								<DropdownMenuTrigger asChild>
									<Button
										...
										aria-label={`${copy.languageLabel}: ${locale.toUpperCase()}`}
									>
										<LanguageIcon />
									</Button>
								</DropdownMenuTrigger>
