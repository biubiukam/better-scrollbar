...
class FenwickTree {
	...
	add(index: number, delta: number) {
		for (let nextIndex = index + 1; nextIndex < this.tree.length; nextIndex += nextIndex & -nextIndex) {
	...
	sumBefore(index: number) {
		let sum = 0
		for (let nextIndex = Math.min(Math.max(index, 0), this.tree.length - 1); nextIndex > 0; nextIndex -= nextIndex & -nextIndex) {
...
class BlockVirtualHeightIndex implements VirtualHeightIndexStore {
	...
	setMeasuredHeight(index: number, height: number) {
		...
		const previousDelta = previousHeight === undefined ? 0 : previousHeight - this.estimatedHeight
	...
	getRange = ({scrollOffset, viewportSize, overscan, overscanPixels, maxItems}: VirtualRangeOptions): VirtualRangeResult => {
		if (this.count === 0) {
			return {
		...
		if (safeOverscanPixels) {
			start = Math.min(
				...
				this.findIndex(visibleEndOffset + safeOverscanPixels.after)
			)
		}
...
export function createVirtualHeightIndexStore(options: VirtualHeightIndexOptions): VirtualHeightIndexStore {
